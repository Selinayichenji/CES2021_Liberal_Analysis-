...
#| eval: true
#| label: fig-percent
#| fig-cap: Percentage of different groups in a category
#| warning: false
...
#origin<-c("Less than $25,000", "$25,000 to $49,999", "$50,000 to $74,999", "$75,000 to $99,999",
...
ggplot(survey_long, aes(x = group, y = vote_percent, color = party, group = party)) +
  geom_line() + 
  geom_point() + 
  scale_color_manual(values = c("vote_liberal" = "red", 
                                "vote_conservative" = "blue", 
                                "vote_NDP" = "orange")) +
  labs(title = "Voting Preferences by Category in Survey",
       x = "Group", 
       y = "Percentage", 
       color = "Party") +
  theme_minimal() +
...
msummary <- modelsummary(
  list("Vote for Liberal Party" = liberal_model),
  statistic = "mad",
  booktabs = TRUE
)

# 然后我们将msummary的结果分为两部分
# 以下是一个如何操作的例子
# 请注意，这需要您手动指定分割点
# 比如，假设我们有10个系数，我们可以在第5个系数处分割
part1 <- msummary[1:11,]
part2 <- msummary[11:22,]

# 创建两个并排的表格
table1 <- kable(part1, booktabs = TRUE) %>%
  kable_styling(font_size = 7) %>%
  pack_rows(index = c("Part 1"))

table2 <- kable(part2, booktabs = TRUE) %>%
  kable_styling(font_size = 7) %>%
  pack_rows(index = c("Part 2"))

# 使用 kableExtra 的两列布局函数
library(kableExtra)
table_combined <- table1 %>% add_indent(levels = 1) %>%
  column_spec(1, width = "5in") %>%
  kable_styling(position = "float_left")

table_combined2 <- table2 %>% add_indent(levels = 1) %>%
  column_spec(1, width = "5in") %>%
